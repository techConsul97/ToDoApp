...
class TasksViewModel @Inject constructor(
    ...
    private val _filterUiInfo = _savedFilterType.map { getFilterUiInfo(it) }.distinctUntilChanged()
     val _searchedTasks:MutableStateFlow<Async<List<Task>>> = MutableStateFlow(Async.Loading)
    ...
    fun getSearchedTasksUsingWord(query:String){
        viewModelScope.launch {
            tasksRepository.getTasksStream().collectLatest{ result ->
                if(result is Success && result.data.isNotEmpty()){
                    val filteredList = result.data.filter { it.description.contains(query) }
                  _searchedTasks.value = Async.Success(filteredList)
              }
            }
        }
    }
